...
def SimpleRecognizePlateWithGui(image, path_save = "", name = ""):
    ...
    if mask == True:
        ...
        if len(path_save) > 0 and len(val) > 0:
            # 保存测试集
            pp.cache.verticalMappingToFolder_segmentation(val[0], path_save, name)
            # 保存训练集
            pp.cache.Perform_save_segmentation_train(val[0], path_save, name)
    # 处理功能子库图片
    else:
        t0 = time.time()
...
class HyperLprWindow(QMainWindow):
    # 定义一个start_init_signal信号，该信号没有参数
    ...
    def initUI(self):
        ...
        # 绑定本类函数rename_current_image_with_info
        ...
        self.hyperlpr_tableview.setEditTriggers(
            QAbstractItemView.NoEditTriggers)
        # 绑定本类函数recognize_one_license_plate
        ...
        # 绑定本类函数select_new_dir，与选择数据集图片库有关
        ...
        # 绑定本类函数batch_recognize_all_images
        self.update_file_path_button.clicked.connect(self.batch_recognize_all_images)
        ...
        line_index += 1
        info_gridlayout.addWidget(segmentation_recognition_label, line_index, 0)
        info_gridlayout.addWidget(self.segmentation_recognition_edit, line_index, 1)
        line_index += 1
        ...
        # image_window_view是HyperLprImageView类的一个实例
    ...
    def select_new_dir(self):
        ...
        :return: None
        '''
        self.hyperlpr_dir_path = QFileDialog.getExistingDirectory(self, "读取文件夹", QDir.currentPath())
    ...
    def scan_files_with_new_dir(self, path):
        '''
        功能：遍历读取到的路径下的所有文件名称，并判断是否为.jpg或者.png图片
        :param path:UI界面选择的图像识别路径
        :return:None
        '''
        # 列出文件夹下所有的目录与文件，输出例如：['OUT', '汽车车牌识别图像库', '测试集']；
        # 或者['粤Z7087澳.jpg', '粤Z7426澳.jpg', '粤Z7442澳.jpg']
        ...
        for i in range(0, len(name_list)):
            ...
            if name_list[i].endswith(".jpg") or name_list[i].endswith(".png"):
                # image_filename_list包含所有.jpg和.png图片的名称
    ...
    def recognize_one_license_plate(self, row, col):
        '''
        功能: 表格控件关联的函数，因为它，所以点击表格控件的每个格子都可以识别车牌
        :param row: row, col会随着点击的表格位置而变化
        :param col:
        :return:
        '''
        if col == 0 and row < len(self.image_filename_list):
            self.current_row = row
            self.recognize_and_show_one_image(self.image_filename_list[row], row)

    def recognize_and_show_one_image(self, image_filename_text, row):
        '''
        功能：识别并显示一张车牌
        :param image_filename_text:
        :param row:
        :return:None
        '''
        if image_filename_text.endswith(".jpg"):
            ...
            image = cv2.imdecode(np.fromfile(path, dtype=np.uint8), -1)
            # SimpleRecognizePlateWithGui函数本文件最前面有定义
    ...
    def batch_recognize_all_images(self):
        ...
        if len(self.subfolder_path) == 0:
            ...
            self.color_correct_number = 0
            self.e2e_recognization_correct_number = 0
            # batch_recognization_thread是LicenseRecognizationThread类的一个实例
            # 设置好批量处理的参数
            self.batch_recognization_thread.set_parameter(self.image_filename_list, self.hyperlpr_dir_path)
            ...
            for i in range(0, len(self.subfolder_path)):          # 当进入计分子库路径时，进行处理
                ...
                # batch_recognization_thread是LicenseRecognizationThread类的一个实例
                # 设置好批量处理的参数
                self.batch_recognization_thread.set_parameter(self.image_filename_list, self.subfolder_path[i])
                self.batch_recognization_thread.run()
            ...
            if Reg_temp[len(Reg_temp) - 1] == Ability_k[0]:
                ...
                for i in range(0, len(Reg_path_file_list)):
                    ...
                    # "大型汽车后牌", "低速车牌", "各类摩托车牌", "挂车牌", "拖拉机牌","武警车牌"，"大型汽车前牌"，"小型汽车牌"
                    # "教练车牌", "新能源-大车牌", "新能源-小车牌"
                    if ((Reg_path_file_list[i] == A_caplat[0]) or (Reg_path_file_list[i] == A_caplat[1]) or
                        ...
                        (Reg_path_file_list[i] == A_caplat[4]) or (Reg_path_file_list[i] == A_caplat[8]) or
                        (Reg_path_file_list[i] == A_caplat[9]) or (Reg_path_file_list[i] == A_caplat[10]) or
                        (Reg_path_file_list[i] == A_caplat[11]) or (Reg_path_file_list[i] == A_caplat[13]) or
                        (Reg_path_file_list[i] == A_caplat[14])):
                        ...
                        for j in range(0, len(Reg_path_sondir_list)):
                            ...
                            if ((Reg_path_file_list[i] == A_caplat[11]) or (Reg_path_file_list[i] == A_caplat[13]) or
                                (Reg_path_file_list[i] == A_caplat[14])):
                    ...
                    # "澳门出入境车牌"，"香港出入境车牌"，"领使馆车牌"，"警用车牌"，"军用车牌"
                    else:
                        print("开始识别" + Reg_path_file_list[i])
                        ...
                        # 识别车牌
                        e2e_img_list, e2e_orin_list, accurace = pred77.Run(Reg_path_sondir + "/")
    ...
    def recognization_done_slot(self, result_list):
        '''
        功能：作为LicenseRecognizationThread类带参信号recognization_done_signal的关联函数
        :param result_list:
        :return:
        '''
        row = result_list[0]
        res_set = result_list[1]
        # fill_table_widget_with_res_info函数用于更新信息
        self.fill_table_widget_with_res_info(res_set, row)
        ...
        if row == len(self.image_filename_list) - 1:
            ...
            self.hyperlpr_tableview.item(total_number,1).setText("{0} / {1} = {2: .3f}".format(
                self.segmentation_recognition_correct_number,total_number,self.segmentation_recognition_correct_number
                                                                          /total_number))
            ...
            self.hyperlpr_tableview.item(total_number, 3).setText("{0} / {1} = {2: .3f}".format(
                self.e2e_recognization_correct_number, total_number,self.e2e_recognization_correct_number
                                                                    / total_number))

            item4 = QTableWidgetItem()
            item4.setTextAlignment(Qt.AlignCenter)
            self.hyperlpr_tableview.setItem(total_number, 4, item4)
            self.hyperlpr_tableview.item(total_number, 4).setText("{0} / {1} = {2: .3f}".format(
                    self.color_correct_number, total_number,self.color_correct_number / total_number))
    ...
    def fill_table_widget_with_res_info(self, res_set, row):
        '''
        功能:更新表格控件里面每张图片对应的相关信息
        :param res_set:
        :param row:对应UI界面表格的行
        :return: None
        '''
...
if __name__ == '__main__':
    ...
    # HyperLprWindow继承了QMainWindow，QMainWindow继承了QWidget，QWidget包含函数showMaximized()

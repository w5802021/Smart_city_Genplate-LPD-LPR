...
############################################################################
#   本文件主要是：分割
#   包含函数：def Getmodel_tensorflow(nb_classes):
...
def slidingWindowsEval(image):
    ...
    if(len(interval) > 3):
        mid = get_median(interval)                                  # 调用get_median函数
    else:
        return []
    ...
    try:
        res = searchOptimalCuttingPoint(image, pin, 0, mid, 3)              # 调用searchOptimalCuttingPoint函数
    except:
        return []
    ...
    refined = refineCrop(seg_block, mid-1)                                 # 调用refineCrop函数

...
def fitLine_ransac(pts,zero_add = 0 ):
    if len(pts) >= 2:
        [vx, vy, x, y] = cv2.fitLine(pts, cv2.DIST_HUBER, 0, 0.01, 0.01)    # 直线拟合
        lefty = int((-x * vy / vx) + y)
...
def findContoursAndDrawBoundingBox(image_rgb):
    ...
    for k in np.linspace(-50, 0, 15):                             # linspace：均分指令
        ...
        binary_niblack = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 17, k)   # AdaptiveThreshold 将灰度图像变换到二值图像；
        imagex, contours, hierarchy = cv2.findContours(binary_niblack.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)      # 寻找图像轮廓
        for contour in contours:
            bdbox = cv2.boundingRect(contour)                                            # 输入二值图，返回四个值，分别是x，y，w，h；x，y是矩阵左上点的坐标，w，h是矩阵的宽和高
    ...
    rgb = cv2.copyMakeBorder(image_rgb, 30, 30, 0, 0, cv2.BORDER_REPLICATE)    # copyMakeBorder：扩充image_rgb的边缘，将图像变大，此处输入为彩色图
    ...
    image = cv2.warpPerspective(rgb, mat, (136, 36), flags=cv2.INTER_CUBIC)    # 对图像进行透视变换
